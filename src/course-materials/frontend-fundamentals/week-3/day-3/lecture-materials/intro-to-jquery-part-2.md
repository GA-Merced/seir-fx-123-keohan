...
For this lesson, we're going to use the same jQuery practice project except we're going to create a `script.js` file inside of a `js` directory to put our JavaScript in:
...
We're all set to get started. Open the preview in a new tab.
...
##### SLACK THREAD TO ANSWER THE FOLLOWING QUESTION
